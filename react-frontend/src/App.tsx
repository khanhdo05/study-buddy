...
import './App.css';
type Page = 'Overview' | 'Study' | 'Practice' | 'Progress' | 'Course settings';
const nav: {
  page: Page;
  icon: string;
}[] = [{
  ...
  return <div className={focus ? 'app focus' : 'app'}>
    ...
    <aside className="sidebar">
      <a className="brand" href="#" onClick={e => {
        e.preventDefault();
        setPage('Overview');
      }}><span className="brand-mark">sb<span>·</span></span>study buddy</a>
    ...
    <div className="page-heading"><div><span className="eyebrow">BIO 101 / {page.toUpperCase()}</span><h1>{page === 'Overview' ? 'A little progress, every day.' : page === 'Study' ? 'Make room for understanding.' : page === 'Practice' ? 'Put your knowledge to work.' : page === 'Progress' ? 'See how far you’ve come.' : 'Your course. Your guidance.'}</h1><p>{page === 'Overview' ? 'Welcome back, Jamie. What will you learn today?' : page === 'Study' ? 'Explore this week’s concepts with your study companion.' : page === 'Practice' ? 'A few thoughtful questions. A stronger understanding.' : page === 'Progress' ? 'Your learning journey, one concept at a time.' : 'Configure the sample learning environment.'}</p></div><span className="semester">FALL 2026</span></div>
    {page === 'Overview' && <>
